...
const AdminProduct = () => {
    ...
    const fetchData = async (page = 1) => {
      ...
      try {
        const response = await api.get(
          `/product/get?page=${page}&limit=${limit}`
        );
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(t("product_question"))) return;
    ...
    <section className="box w-full grid gap-4">
      ...
      <div className="w-full overflow-x-auto">
        <div className="min-w-[800px]">
          <div className="w-full grid grid-cols-8 gap-4 bg-primary rounded-sm">
          ...
          {pageData.length > 0 ? (
            pageData.map((data, index) => (
              <div
                key={data._id}
                className="w-full grid grid-cols-8 gap-4 items-center odd:bg-gray-100 even:bg-white rounded-sm"
              >
                ...
                <p className="text-textColor text-sm p-2 text-center">
                  {language === "en"
                    ...
                    : data.category?.nameTm}
                </p>
                <div className="flex items-center justify-center gap-2 text-center">
                  <Link
                    to={`/admin/product/update/${data._id}`}
                    className="btn"
                  >
                    <MdModeEditOutline />
                  </Link>
